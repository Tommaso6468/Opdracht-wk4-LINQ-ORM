...
class Program
{
    ...
    public static async Task Main(string[] args)
    {
        ...
        using (DatabaseContext c = new DatabaseContext())
        {
            ...
            Gast testGast = new Gast("testMail@gmail.com") { GeboorteDatum = new DateTime(2000, 3, 12), Credits = 5};
            DateTimeBereik testDate = new DateTimeBereik() { Begin = new DateTime(2022, 6, 2), Eind = new DateTime(2022, 6, 3) };
            c.Gasten.Add(testGast);
            c.SaveChanges();
            await c.Boek(testGast, c.Attracties.First(), testDate);
            // List<Reservering> res = c.Gasten.FirstOrDefault(item => item.GebruikerId == testGast.GebruikerId).Reserveringen.ToList();
