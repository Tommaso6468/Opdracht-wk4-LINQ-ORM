...
class DemografischRapport : Rapport
{
    ...
    private async Task<bool> AlleGastenHebbenReservering() => context.Gasten.All(g => g.Reserveringen != null);
    // private async Task<int> AantalSinds(DateTime sinds) => /* ... */;
    private async Task<Gast> GastBijEmail(string email) => context.Gasten.FirstOrDefault(g => g.Email == email);
    private async Task<Gast?> GastBijGeboorteDatum(DateTime d) => context.Gasten.FirstOrDefault(g => g.GeboorteDatum == d);
    private async Task<double> PercentageBejaarden()  {
        var bejaarden = context.Gasten.Where(g => g.GeboorteDatum < DateTime.Now.AddYears(-80));
        var totaal = context.Gasten.Count();
        return bejaarden.Count() / (double)totaal;
    }
    private async Task<int> HoogsteLeeftijd() => context.Gasten.Max(g => DateTime.Now.Year - g.GeboorteDatum.Year);
    private async Task<IEnumerable<Gast>> Blut(IEnumerable<Gast> g) => g.Where(gast => gast.Credits < 0);
    private async Task<List<(Gast, int)>> GastenMetActiviteit(IEnumerable<Gast> gasten) => gasten.Select(g => (g, g.Reserveringen.Count())).ToList();
